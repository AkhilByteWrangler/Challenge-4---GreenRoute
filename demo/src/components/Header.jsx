import { Sun, Moon, Pause, Play, RotateCcw, Brain, Clock, ChevronDown } from 'lucide-react';
...
export default function Header({ simState, paused, speed, setSpeed, togglePause, reset, policyLoaded, onShowTraining, darkMode, setDarkMode, agents = ['PPO', 'Q-Learning', 'DQN', 'Random', 'Greedy'], selectedAgent = 'PPO', onAgentChange = () => {} }) {
  ...
  return (
    <header className="flex items-center justify-between px-6 py-3 border-b border-slate-200 dark:border-white/[0.1] bg-white dark:bg-slate-950">
      {/* Logo */}
        ...
        <div className="relative z-50">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-[11px] font-medium border transition-all
              ${policyLoaded
                ? 'bg-green-100 dark:bg-green-950/40 border-green-300 dark:border-accent-green/30 text-green-700 dark:text-accent-green hover:bg-green-200 dark:hover:bg-green-950/60'
                : 'bg-amber-100 dark:bg-amber-950/40 border-amber-300 dark:border-accent-amber/30 text-amber-700 dark:text-accent-amber'
              }`}
          >
            <Brain className="w-3.5 h-3.5 flex-shrink-0" />
            <span className="hidden sm:inline">{selectedAgent}</span>
            <ChevronDown className={`w-3 h-3 flex-shrink-0 transition-transform ${agentDropdownOpen ? 'rotate-180' : ''}`} />
          </button>

          {agentDropdownOpen && policyLoaded && (
            <div className="absolute top-full left-0 mt-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-white/[0.1] rounded shadow-lg z-50">
              {agents.map((agent) => (
                <button
                  key={agent}
                  onClick={() => {
                  ...
                  className={`block w-full text-left px-4 py-2 text-sm transition-colors
                    ${selectedAgent === agent
                      ? 'bg-green-100 dark:bg-green-950/40 text-green-700 dark:text-accent-green font-medium'
